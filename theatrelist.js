...
function popular(data,theatreId=null) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if(theatreId==null){
            div.setAttribute('href','viewmoviedetails.html?movieId='+data[i].id);//todo
        }
        else{
            div.setAttribute('href','viewmoviedetails.html?movieId='+data[i].id+'&theatreId='+ theatreId);
        }
    ...
    count.innerHTML ="<b>Results:</b>" + data.length;
...
$('#theatre-select').on('change', function (e) {
    ...
    let moviesMap =  JSON.parse(localStorage.getItem("MOVIES"));
    let movies = moviesMap [ value];
    var mainContainer = document.getElementById('movie-select');
    document.querySelector("#movie-select").innerHTML = "";
    mainContainer.setAttribute('class', 'movie-recommendation');
    var data = movies;//getTheatreMovies(value);//['' + value + ''];
    popular(data,value);
    for (var i = 0; i < data.length; i++) {
        var div = document.createElement("a");
        div.setAttribute('class', 'movie-view');
